...
void readMessage() {
	...
	if (msgNew) {
		for (size_t id = 0; id < MSG_BUFFERLEN/2; id++) {
			Serial.print(msg[id]);
		}
		Serial.println(' ');
		updateRobot(msg);
...
void updateRobot(uint8_t *msg) {
	...
	for (size_t idx = 0; idx < msg[idxMsgLen] - 1; idx += PARAMETER_BYTE_LEN + 1) {
		...
		switch (robotMode) {
			case MODE_INIT:
			{
				robotParameters[(size_t)msg[idx + idxParamStart]] = convertBytes2Float(buffer);
				break;
			}
